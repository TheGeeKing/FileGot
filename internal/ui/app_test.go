...
func TestMainWindowPresentation(t *testing.T) {
	...
	if rows != 4 || columns != 3 || !application.table.ShowHeaderRow {
		t.Fatalf("compact table = %dx%d, sticky header %t; want 4x3 with header", rows, columns, application.table.ShowHeaderRow)
	}
	...
	application.table.UpdateCell(widget.TableCellID{Row: 1, Col: 1}, cell)
	...
	for row, file := range application.files {
		cell := application.table.CreateCell()
		application.table.UpdateCell(widget.TableCellID{Row: row, Col: 0}, cell)
	...
	application.table.UpdateCell(widget.TableCellID{Row: 0, Col: 0}, lightCell)
	lightBackground, _ := fileCellParts(lightCell)
	light := lightBackground.FillColor
	app.Settings().SetTheme(theme.DarkTheme())
	darkCell := application.table.CreateCell()
	application.table.UpdateCell(widget.TableCellID{Row: 0, Col: 0}, darkCell)
	...
	application.table.Select(widget.TableCellID{Row: 1, Col: 0})
	...
	application.table.UpdateCell(widget.TableCellID{Row: 1, Col: 2}, selectedCell)
