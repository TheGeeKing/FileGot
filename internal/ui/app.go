...
func (application *Application) build() {
	application.table = widget.NewTable(
		func() (int, int) { return len(application.files), 3 },
		...
		func(id widget.TableCellID, object fyne.CanvasObject) {
			...
			file := application.files[id.Row]
			if id.Row == application.selected {
	...
	application.table.ShowHeaderRow = true
	application.table.CreateHeader = func() fyne.CanvasObject {
		label := widget.NewLabel("")
		label.SizeName = theme.SizeNameCaptionText
	...
	application.table.UpdateHeader = func(id widget.TableCellID, object fyne.CanvasObject) {
		object.(*widget.Label).SetText([]string{"Original File", "Status", "Proposed Name"}[id.Col])
	}
	application.table.OnSelected = func(id widget.TableCellID) {
		application.selected = id.Row
